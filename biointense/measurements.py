...
class ode_measurements(object):
    ...
    def __init__(self, measdata, indep=None, *args, **kwargs):
        '''
        '''
...
#                        print('Attention: Information for xdata is not explicitly set!')
        ...
        if isinstance(measdata, pd.DataFrame):
            self.Data = measdata.reset_index(drop=True)
...
#            self.Data_dict[var] = pd.DataFrame(self.Data[var].dropna())
    ...
    @property
    def _Error_Covariance_Matrix_PD(self):
        ECM_PD = self._uncertainty.get_uncertainty(self.Data)

        ECM_PD = ECM_PD*self.Data.applymap(lambda x: np.nan if math.isnan(x) else 1)
        ECM_PD = ECM_PD.applymap(lambda x: 1e50 if math.isnan(x) else x)
        return ECM_PD

    def _index_to_dict(self, index, indep):
        """
        """
        independent_dict = {}
        if indep is None:
